{foreach from=$accessories item=accessory name=accessories_list}
    {if $accessory->available_for_order && !isset($restricted_country_mode)}
        ...
        <li class="afc-template afc-template-{$accessory->id_product_attribute_parent} col-xs-6 col-sm-4 col-md-3 item product-box ajax_block_product{if $smarty.foreach.accessories_list.first} first_item{elseif $smarty.foreach.accessories_list.last} last_item{else} item{/if} product_accessories_description height-fix">
            <div class="product_desc">
                <a href="{$accessoryLink|escape:'html':'UTF-8'}" title="{$accessory->legend|default|escape:'html':'UTF-8'}" class="product-image product_image">
                    <img class="lazyOwl" src="{$link->getImageLink($accessory->link_rewrite, $accessory->images[0]['id_image'], 'home_default')|escape:'html':'UTF-8'}" alt="{$accessory->legend|default|escape:'html':'UTF-8'}" />
                </a>
